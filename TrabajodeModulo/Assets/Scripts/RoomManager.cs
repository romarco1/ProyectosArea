...
public class RoomManager   {
	...
	public void SetCurrentRoom(int j, int i) {
		...
		if (maproom!=null) 
			maproom.transform.Find("p").GetComponent<Renderer>().enabled = false;
		...
		maproom.transform.Find("p").GetComponent<Renderer>().enabled = true;
	...
	public bool CanGo(MOVEMENTDIRECTION dir,int j,int i) {
		...
		case MOVEMENTDIRECTION.UP: 								
			return rooms[j,i].transform.Find("door_top")!=null;
		case MOVEMENTDIRECTION.DOWN: 
			return rooms[j,i].transform.Find("door_bottom")!=null;
		case MOVEMENTDIRECTION.LEFT: 
			return rooms[j,i].transform.Find("door_left")!=null;
		case MOVEMENTDIRECTION.RIGHT: 
			return rooms[j,i].transform.Find("door_right")!=null;		
		}
	...
	public void Setup() {
		...
		for (int j = 0;j<Max_j;j++) {
			for (int i = 0;i<Max_i;i++) {
				...
				if (room!=null) {
					if (!CanGo(MOVEMENTDIRECTION.UP, j,i)) maproom.transform.Find("top").gameObject.SetActive(false);
					if (!CanGo(MOVEMENTDIRECTION.DOWN, j,i)) maproom.transform.Find("bottom").gameObject.SetActive(false);
					if (!CanGo(MOVEMENTDIRECTION.LEFT, j,i)) maproom.transform.Find("left").gameObject.SetActive(false);
					if (!CanGo(MOVEMENTDIRECTION.RIGHT, j,i)) maproom.transform.Find("right").gameObject.SetActive(false);
